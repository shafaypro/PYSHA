...
class db_data:
    ...
    def create_database (self):
        self.cur.execute(
            '''CREATE TABLE IF NOT EXISTS Requests(r_id INTEGER PRIMARY KEY AUTOINCREMENT,r_text TEXT,r_responce TEXT)''')
        self.cur.execute(
            '''CREATE TABLE IF NOT EXISTS Reminders(Rem_id INTEGER  PRIMARY KEY AUTOINCREMENT,Rem_name TEXT,Rem_Date DATE)''')
        self.cur.execute(
            '''CREATE TABLE IF NOT EXISTS HISTORY(H_id INTEGER PRIMARY KEY AUTOINCREMENT, H_request_Text TEXT,H_Date DATE DEFAULT CURRENT_TIMESTAMP)''')
