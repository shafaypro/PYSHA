    ...
    try:
        ...
        if gender is not "":
            if str(gender).lower() == "male":
                ...
                choose_voice_id = random.randint(1,2)  # finding the random number , as we want the females to be random
                print("Voice id changing to : ", choose_voice_id)
                Engine_updated.setProperty('voice', Engine_voices[choose_voice_id].id)  # setting the voice property of the Engine
...
def change_byname(name=""):
    ...
    if name  is not "":
        if str(name).lower() == "david":  # David Voice
            try:
                Engine_updated.setProperty('voice', Engine_voices[0].id)  # sets the voice to david, as it is on the index 0
            except Exception as E:
                print("Unable to change the voice to david, David not found :",E)
        elif str(name).lower() == "hazel":  # hazel Voice
            try:
                Engine_updated.setProperty('voice', Engine_voices[1].id)  # sets the voice to hazel since the hazel is on the index 1
            except Exception as E:
                print("Unable to change the voice to hazel, hazel is not found :", E)  # Debugging purpose
        elif str(name).lower() == "zira":   # Zira Voice
            try:
                Engine_updated.setProperty('voice', Engine_voices[2].id)  # sets the voice to the Zira, its on the index 2
            except Exception as E:
...
def volume_update(status_value = "high"):
    ...
    if status_value != "high" and (Engine_volume > 0.0 and Engine_volume <= 1.0):
...
# engines.say("The quick brown fox jumped over the lazy dog")
# engines.runAndWait()
'''-- Below is the checking of the change of the volumes , rather increasing or decreasing the volumd of the assistant'''
