...
class WebScrap:
    ...
    def scrap_link (self, link):
        ...
        for script in Beautified_soup(["script", '[document]', 'head', 'title',
                                       "style"]):  # to remove all the scripts from the required page.
        ...
        filtered_text = [T for T in text if len(T) > 50]  # Filtering the texts, which are greater than 50 characters
        print(filtered_text)  # printing the filtered text
        # print(text)
        # for l in text:
        #     if l.startswith("'b"):
        #         l = l.replace("'b", '')
        ...
        #         pass
        #     print(l)
        WRP = WebReader_PYSHA()  # Creating the object of the webreader Pysah
        WRP.read_web_text_list(filtered_text)  # screapping the list for the filtered web
